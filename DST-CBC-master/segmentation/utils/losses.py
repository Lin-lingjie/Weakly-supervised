...
class DynamicMutualLoss(_WeightedLoss):
    ...
    def forward(self, inputs, targets, split_index=None):
        ...
        if split_index is None or (self.gamma1 == 0 and self.gamma2 == 0):  # No dynamic loss
            ...
            decision_current = outputs.argmax(1)  # N 在维度1上的最大值，表示当前输出的决策
            decision_pseudo = real_targets.clone().detach()  # N  真是目标作为决策
            confidence_current = outputs.max(1).values  # N 输出的自信度

            temp = real_targets.unsqueeze(1).clone().detach()  # 扩展real_targets 张量的维度，在新增加的维度上为 1
            temp[temp == self.ignore_index] = 0  #
            probabilities_current = outputs.gather(dim=1, index=temp).squeeze(1)  #获取对应位置的概率值

            confidence_pseudo = targets[:, 1, :, :].clone().detach()  # N  获取伪决策的置信度
